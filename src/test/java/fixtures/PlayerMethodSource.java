...
public class PlayerMethodSource {
    ...
    private static Stream<Arguments> wrongNumberOfPlayers() {
        try {
            return Stream.of(
                    arguments(Collections.singletonList(of())),
                    arguments(asList(of(), of(), of())));
        } catch (Exception e) {
            return Stream.empty();
        }
    }
    ...
    private static Stream<Arguments> tournament() {
        try {
            return Stream.of(
                    ...
                    arguments(asList(rockPlayer, scissorsPlayer, paperPlayer, scissorsPlayer, scissorsPlayer), rockPlayer),
                    arguments(asList(scissorsPlayer, paperPlayer, rockPlayer, paperPlayer, paperPlayer, scissorsPlayer), scissorsPlayer),
                    arguments(asList(paperPlayer, rockPlayer, rockPlayer, scissorsPlayer, rockPlayer, scissorsPlayer, rockPlayer), paperPlayer));
        } catch (Exception e) {
            return Stream.empty();
        }
    }
